...
@section('title', 'Ubah Relawan')

@section('content')
    <div class="inst-form-page" style="max-width:52rem;">
        <a href="{{ route('master.relawan.index') }}" class="inst-back">← Kembali ke daftar</a>
        <h2 class="inst-form-title">Ubah relawan</h2>
        <p class="inst-form-lead">Perbarui data relawan. Unggah foto baru untuk mengganti (opsional).</p>
        <div class="inst-form-card">
            <form id="form-relawan" method="POST" action="{{ route('master.relawan.update', $relawan) }}" enctype="multipart/form-data" class="space-y-6">
                @csrf
                @method('PUT')
                @include('relawan._form', ['relawan' => $relawan, 'posisis' => $posisis])
                <div class="flex flex-wrap gap-3 pt-2">
                    <button type="submit" class="inst-btn-primary">Simpan</button>
                    <a href="{{ route('master.relawan.show', $relawan) }}" class="inst-btn-outline">Profil</a>
