...
@section('content')
    <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
            <h2 class="inst-page-title">Kategori limbah</h2>
            <p class="inst-page-desc">Master kategori untuk pencatatan limbah dapur.</p>
        </div>
        @if (auth()->user()->hasAnyRole(['super_admin', 'admin_pusat']))
            <a href="{{ route('master.kategori-limbah.create') }}" class="inst-btn-primary shrink-0">
    ...
    <form method="GET" action="{{ route('master.kategori-limbah.index') }}" class="inst-filter-panel mb-4">
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-3 sm:items-end">
            ...
            <div class="flex flex-wrap gap-2">
                ...
                <a href="{{ route('master.kategori-limbah.index') }}" class="inst-btn-outline">Reset</a>
            </div>
        </div>
    </form>
                ...
                <tbody>
                    @forelse ($items as $row)
                        <tr>
                            ...
                            <td class="text-right">
                                @if (auth()->user()->hasAnyRole(['super_admin', 'admin_pusat']))
                                    <a href="{{ route('master.kategori-limbah.edit', $row) }}" class="text-xs font-semibold" style="color:#4a9b7a;">Ubah</a>
                                    <form method="POST" action="{{ route('master.kategori-limbah.destroy', $row) }}" class="ml-3 inline form-hapus-kategori">
                                        @csrf
                                        @method('DELETE')
