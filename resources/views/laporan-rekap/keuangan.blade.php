...
@section('content')
    @include('laporan-rekap._tabs', ['tab' => 'keuangan'])

    <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
            <h2 class="inst-page-title">Laporan keuangan (neraca)</h2>
            <p class="inst-page-desc">{{ $data['mulai']->translatedFormat('F Y') }} — {{ $namaDapur }}</p>
        </div>
        <div class="flex flex-wrap gap-2">
            <a href="{{ route('laporan-rekap.keuangan.export-excel') }}" class="inst-btn-outline">Excel</a>
            <a href="{{ route('laporan-rekap.keuangan.export-pdf') }}" target="_blank" class="inst-btn-outline">PDF</a>
            <a href="{{ route('laporan-rekap.keuangan.export-pdf', ['draft' => 1]) }}" target="_blank" class="inst-btn-outline">PDF (draft)</a>
        </div>
    </div>

    <form method="GET" action="{{ route('laporan-rekap.keuangan') }}" class="inst-panel mb-6 grid gap-4 p-4 sm:grid-cols-2 lg:grid-cols-4 sm:p-6">
        <div>
    ...
    <div class="grid gap-6 lg:grid-cols-2">
        <div class="inst-panel p-4 sm:p-6">
            ...
            <table class="inst-table">
                <thead><tr><th>Akun (Jenis Dana)</th><th class="text-right">Jumlah</th></tr></thead>
                <tbody>
